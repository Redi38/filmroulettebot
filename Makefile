...
.PHONY: help venv install install-dev up web down logs ps \
        ci ci-compile ci-test ci-js test compile js-syntax clean
...
ci: ci-compile ci-test ci-js ## Run every CI stage locally (compile + pytest + JS syntax)
...
# Short aliases for the two most-used ci-* stages
