"""Unit tests for app/web/server/shared/ — the bounded in-memory state,
...
from app.web.server.shared.constants import CATEGORIES, ROULETTE_CATEGORIES, SPIN_COOLDOWN
from app.web.server.shared.spin_state import (
    _BoundedDict,
    _last_spin_at,
...
from app.web.server.shared.validation import check_category
...
def test_check_category_rejects_unknown_category():
    with pytest.raises(HTTPException) as exc_info:
        check_category("definitely-not-a-category")
    assert exc_info.value.status_code == 404


def test_check_category_accepts_every_known_category():
    for cat in CATEGORIES:
        check_category(cat)  # must not raise
...
def test_spin_cooldown_blocks_immediate_second_spin():
    ip = "1.2.3.4"
    check_spin_cooldown(ip)  # first spin: fine
    with pytest.raises(HTTPException) as exc_info:
...
def test_spin_cooldown_is_per_client_ip():
    check_spin_cooldown("1.1.1.1")
    check_spin_cooldown("2.2.2.2")  # different client, no cooldown yet
...
def test_spin_cooldown_allows_spin_again_once_elapsed(monkeypatch):
    ...
    check_spin_cooldown(ip)  # must not raise
...
def test_spin_cooldown_state_is_bounded(monkeypatch):
    ...
    from app.web.server.shared.spin_state import _SPIN_STATE_MAX_ENTRIES

    fake_now = [0.0]
    monkeypatch.setattr(time, "monotonic", lambda: fake_now[0])
    for i in range(_SPIN_STATE_MAX_ENTRIES + 500):
        fake_now[0] += SPIN_COOLDOWN + 0.01
        check_spin_cooldown(f"10.0.0.{i}")
...
def test_pick_title_does_not_immediately_repeat():
    items = ["A", "B"]
    first = pick_title_for_client("client-1", "movies", items)
    second = pick_title_for_client("client-1", "movies", items)
    assert first != second


def test_pick_title_is_scoped_per_client_and_category():
    items = ["A", "B", "C"]
    pick_title_for_client("client-1", "movies", items)
    ...
    assert pick_title_for_client("client-2", "movies", items) in items
    assert pick_title_for_client("client-1", "series", items) in items
...
def test_build_wheel_pool_caps_size_for_large_lists():
    ...
    winner = items[0]
    pool, weights = build_wheel_pool(items, winner, size=120)
    assert len(pool) == 120
...
def test_build_wheel_pool_unweighted_gives_equal_weights():
    items = ["A", "B", "C"]
    _, weights = build_wheel_pool(items, "A", weighted=False)
    assert weights == [1] * len(weights)


def test_build_wheel_pool_weighted_gives_earlier_items_more_weight():
    items = ["A", "B", "C"]
    pool, weights = build_wheel_pool(items, "A", weighted=True)
...
def test_random_entries_flatten_every_list_with_flat_weights():
    entries, weights = random_entries({"movies": ["A", "B"], "series": ["C"]})
...
def test_random_entries_weighted_gives_the_same_position_the_same_weight_in_every_list():
    entries, weights = random_entries(
...
def test_random_entries_weighted_lot_weighs_a_first_position():
    entries, weights = random_entries({"movies": ["A", "B", "C"]}, weighted=True, lots={"marvel": "X"})
...
def test_random_entries_weighted_lots_alone_still_have_a_positive_weight():
    _, weights = random_entries({}, weighted=True, lots={"marvel": "X", "dc": "Y"})
    assert weights == [1, 1]


def test_build_random_wheel_pool_holds_every_title_from_every_list():
    entries, weights = random_entries({"movies": ["A", "B"], "cartoons": ["C"], "series": ["D"]})
    pool, pool_weights, _, _ = build_random_wheel_pool(entries, weights, entries[-1])
...
def test_build_random_wheel_pool_keeps_weights_aligned_with_titles():
    entries, weights = random_entries({"movies": ["A", "B", "C"]}, weighted=True)
    pool, pool_weights, winner_index, _ = build_random_wheel_pool(entries, weights)
...
def test_build_random_wheel_pool_winner_index_points_at_the_winners_segment():
    entries, weights = random_entries({"movies": ["A", "B", "C"]}, lots={"marvel": "Железный человек"})
    for winner in entries:
        pool, _, winner_index, _ = build_random_wheel_pool(entries, weights, winner)
        assert pool[winner_index] == winner.label


def test_build_random_wheel_pool_draws_a_lot_by_franchise_name_not_by_its_title():
    entries, weights = random_entries({"movies": ["A"]}, lots={"marvel": "Железный человек"})
    pool, _, _, _ = build_random_wheel_pool(entries, weights, entries[-1])
...
def test_build_random_wheel_pool_caps_size_and_keeps_the_winner():
    ...
    entries, weights = random_entries({"movies": titles})
    pool, pool_weights, winner_index, _ = build_random_wheel_pool(entries, weights, entries[0], size=10)
...
def test_build_random_wheel_pool_without_winner_still_caps_size():
    entries, weights = random_entries({"movies": [f"t{i}" for i in range(50)]})
    pool, _, _, _ = build_random_wheel_pool(entries, weights, None, size=10)
    assert len(pool) == 10


def test_pick_random_entry_returns_an_entry_from_any_list():
    entries, weights = random_entries({"movies": ["A"], "series": ["B"]}, lots={"dc": "Бэтмен"})
    for client in ("c1", "c2", "c3"):
        assert pick_random_entry(client, "random", entries, weights) in entries


def test_pick_random_entry_can_land_on_a_lot():
    entries, weights = random_entries({"movies": ["A"]}, lots={"marvel": "Железный человек"})
    picked = {pick_random_entry(f"c{i}", "random", entries, weights).cat for i in range(60)}
    assert picked == {"movies", "marvel"}


def test_pick_random_entry_does_not_repeat_the_previous_title():
    entries, weights = random_entries({"movies": ["A", "B"]})
    first = pick_random_entry("repeat-client", "random", entries, weights)
    for _ in range(20):
        nxt = pick_random_entry("repeat-client", "random", entries, weights)
        assert nxt != first
        first = nxt


def test_pick_random_entry_with_a_single_entry_may_repeat():
    entries, weights = random_entries({"movies": ["Only"]})
    assert pick_random_entry("solo", "movies", entries, weights).title == "Only"
    assert pick_random_entry("solo", "movies", entries, weights).title == "Only"


def test_random_pool_weights_normal_mode_is_flat():
    assert random_pool_weights({"movies": ["A", "B"]}, ["B", "A"]) == [1, 1]


def test_random_pool_weights_weighted_follows_the_pool_order():
    weights = random_pool_weights({"movies": ["A", "B", "C"]}, ["C", "A", "B"], weighted=True)
...
def test_random_pool_weights_weighted_matches_random_entries_across_lists():
    ...
    lots = {"marvel": "X"}
    entries, weights = random_entries(lists, weighted=True, lots=lots)
    pool = [e.label for e in entries]
    assert random_pool_weights(lists, pool, weighted=True, lots=lots) == weights
